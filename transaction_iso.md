# 低代码平台整理之事务隔离级别原理是怎样的？

低代码平台往往场景比较复杂，单一数据存储不一定能支持好很多功能，可能会涉及到多数据源存储，这里面就会涉及到事务隔离，平常我们也至多至少了解事务隔离这块，这里就完整的整理下整个事务隔离的原理机制是怎样的；

## 事务有哪些特性？

不是所有的存储都支持事务，很多存储是不支持事务的，就算同一个存储也会有事务非事务引擎，比如mysql的innodb与myisam；

事务看起来比较简单，但是要实现事务必须要遵守4个特性，分别是原子性，一致性，隔离性，持久性，这里就不细细提到，重点讲一下隔离性这块，隔离性指的是允许多个并发事务同时对其数据进行读写和修改的能力，隔离性可以防止多个事务并发执行时由于交叉执行而导致数据不一致，因为多个事务同时使用相同的数据时，不会相互干扰，每个事务都有一个完整的数据空间，对其他并发事务是隔离的。也就是说，消费者购买商品这个事务，是不影响其他消费者购买的。
...
## [#](https://xiaolincoding.com/mysql/transaction/mvcc.html#读提交是如何工作的)读提交是如何工作的？
...
## [#](https://xiaolincoding.com/mysql/transaction/mvcc.html#总结)总结
